...
const Hero = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.timeline().fromTo(".name-animation", {x: -100, opacity: 0, rotate: -10}, {x: 0, opacity: 1, rotate: 0})
    }, component);

    return () => ctx.revert();
  }, []);
  return (
    <section id="hero" ref={component}>
      <div className="grid min-h-[70vh] grid-cols-1 md:grid-cols-2 items-center">
        <div className="col-start-1 md:row-start-1">
          <h1 className="mb-8 text-[clamp(3rem,20vmin,9rem)] font-extrabold leading-none tracking-tighter" aria-label="Tim Mauersberger">
